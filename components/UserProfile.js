...
const UserProfile = ({ item, userId }) => {
  ...
  const sendConnectionRequest = async (currentUserId, selectedUserId) => {
    console.log("Request initiated with:", currentUserId, selectedUserId);

    try {
      const response = await fetch("http://192.168.112.27:3000/connection-request", {
        method: "POST",
        headers: {
    ...
    >
      ...
      <Pressable
        onPress={() => {
          ...
          sendConnectionRequest(userId, item._id);
        }}
        style={{
          marginLeft: "auto",
          marginRight: "auto",
